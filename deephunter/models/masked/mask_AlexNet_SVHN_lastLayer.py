...
import torch
class mask_AlexNet_lastLayer(nn.Module):

    def __init__(self, num_classes=10):
        super(mask_AlexNet_lastLayer, self).__init__()
    ...
    def forward(self, x, sensUnits):
        ...
        mask = torch.ones_like(feat_conv1_relu )
        sens = torch.tensor(sensUnits[0])
#         if len(sensUnits[0]) != 0:
#             mask[:, sens, :, :] = 0
        feat_conv1_relu  = feat_conv1_relu .mul(mask)
...
#         print(feat_conv1_relu1.size())
        ...
        mask = torch.ones_like(feat_conv2_relu )
        sens = torch.tensor(sensUnits[1])
#         if len(sensUnits[1]) != 0:
#             mask[:, sens, :, :] = 0
        feat_conv2_relu  = feat_conv2_relu .mul(mask)
...
#         print(feat_conv2_relu2.size())
        ...
        mask = torch.ones_like(feat_conv3_relu )
        sens = torch.tensor(sensUnits[2])
#         if len(sensUnits[2]) != 0:
#             mask[:, sens, :, :] = 0
        feat_conv3_relu  = feat_conv3_relu .mul(mask)
        
#         print(feat_conv3_relu3.size())
        feat_conv4 = self.features[8](feat_conv3_relu)
        feat_conv4_relu = self.features[9](feat_conv4)
        
        mask = torch.ones_like(feat_conv4_relu)
        sens = torch.tensor(sensUnits[3])
#         if len(sensUnits[3]) != 0:
#             mask[:, sens, :, :] = 0
        feat_conv4_relu  = feat_conv4_relu .mul(mask)
#         print(feat_conv4_relu4.size())

        feat_conv5 = self.features[10](feat_conv4_relu)
        feat_conv5_relu = self.features[11](feat_conv5)
        
        mask = torch.ones_like(feat_conv5_relu )
        sens = torch.tensor(sensUnits[4])
        if len(sensUnits[4]) != 0:
            mask[:, sens, :, :] = 0
        feat_conv5_relu  = feat_conv5_relu .mul(mask)
