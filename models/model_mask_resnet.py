...
__all__ = ['ResNet', 'resnet20', 'resnet32', 'resnet44', 'mask_resnet56', 'resnet110', 'resnet1202']
...
class ResNet(nn.Module):
    ...
    def forward(self, x, sensUnits, all_blocks=True):
        index = 1
        outputs = {}
        out = F.relu(self.bn1(self.conv1(x)))
        output_name = "feat_conv" +str(index) + "_relu" 
        ...
        if all_blocks:       
            ...
            out = self.layer1(out)
            outputs["feat_conv2_relu"] = out
            out = self.layer2(out)
            outputs["feat_conv3_relu"] = out
            out = self.layer3(out)
            outputs["feat_conv4_relu"] = out
