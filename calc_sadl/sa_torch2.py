...
#     )
    ...
    if is_classification:
        p = Pool(num_proc)
        print(prefix + "Model serving",dataset.shape)
...
#         print ("l1ayer_outputs",type(layer_outputs))
#         print ("!!!!"*8, len(layer_outputs))
        
        #layer_outputs list ,len=1
        #first : (60000, 128) float32
#         print (prefix+"!!!!ccccc",type(layer_outputs),len(layer_outputs))
#         layer_outputs_d = layer_outputs[0]
#         
#         print (prefix+"!!!!eeee",layer_outputs_d.shape,layer_outputs_d.dtype)
#         
...
#         print ("----"*8)
#         print (ats.shape)
#         print ("type ats0",type(ats[0]))
#         print ("type ats0--- ",ats[0].shape)
#         
#         print ("type ats1",type(ats[1]))
#         print ("type ats1--- ",ats[1].shape)
#         
#         print ("type ats00011111--- ",ats.dtype)
#         
# #         print ("type ats1-3",type(ats[1]["activation_3"]))
# #         print ("type ats1-3-- ",ats[1]["activation_3"].shape)
#         
#         print ("--->pred-->",type(pred))
#         print ("--->pred-->len:",len(pred))
#         
#         print ("type pred",type(pred[0]))
#         print ("type pred--- ",pred[0].shape)
# 
#         print ("type 000000111111111--- ",pred.dtype)
...
#         train_ats = np.load(saved_train_path[0],allow_pickle=True)
#         train_pred = np.load(saved_train_path[1],allow_pickle=True)
#     else:
    if 1==1:
...
#         target_ats = np.load(saved_target_path[0],allow_pickle=True)
#         target_pred = np.load(saved_target_path[1],allow_pickle=True)
#     else:
    if 1==1:
...
def fetch_evluate(model,x_train=None,y_train=None,x_test=None,y_test=None,batch_size=512):
    device=torch.device( 'cuda' if torch.cuda.is_available() else 'cpu')

    def do_evaluate(model,testloader):
        model.eval()
        test_loss = 0
        correct = 0
        total = 0
    ...
    if x_train is not None :
        train_dt = torch.utils.data.TensorDataset(torch.from_numpy(x_train), torch.from_numpy(y_train), )
        train_dl = torch.utils.data.DataLoader(train_dt,batch_size=batch_size,num_workers=4,shuffle=False)
        train_acc = do_evaluate(model, train_dl)
    
    if x_test is not None :
        test_dt = torch.utils.data.TensorDataset(torch.from_numpy(x_train), torch.from_numpy(y_train), )
        test_dl = torch.utils.data.DataLoader(test_dt,batch_size=batch_size,num_workers=4,shuffle=False)
        test_acc = do_evaluate(model, test_dl)
