...
#         n_bucket = args.n_bucket
        for  args_attack in  ["pgd", "manu_100_adv", "manu_100_nature"]:
...
#         exit()
        ...
        for  args_attack in  ["pgd", "manu_100_adv", "manu_100_nature", "manu_100_nature2", "manu_100_nature3"]:
            key = [args.d, args_attack]
            key= ",".join(key)
            results_lsa[args_attack] = []
            results_dsa[args_attack] = []
            ...
            if args.nma:
                ...
                for idx, data_dict in enumerate(dl):
                    ...
                    for layer in range(num_layer):
                        ...
                        target_nma, clu_all = fetch_newMetric(model, ori_model, x_train, x_test, y_test, sample_threshold,
                                               target_name, [layer_names[layer]], args, cluster_paths, layer, samples_clusters, fakePath=False, rest=args.rest)
#                         print(target_nma)
...
#                         buckets = np.digitize(target_nma, np.linspace(np.amin(target_nma), upper[layer], args.n_bucket))
                        ...
                        buckets = [str(buckets[i]) + "_" +  str(clu_all[i]) for i in range(len(buckets))]
                        buckets_every_layer.append(buckets)
                    for i in range(len(buckets_every_layer[0])):
                        for layer in range(num_layer):   
                            name = str(buckets_every_layer[layer][i]) + '-'
                        covered.add(name)
                    target_cov = len(covered) / (float(n_bucket) * num_layer * num_cluster) * 100
