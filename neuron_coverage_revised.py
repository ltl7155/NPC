...
#                 print("cluster path", path)
                dis = self.cal_path_similaity(path_x[index], path, simi_soft=simi_soft)[1]
                if i == 0 or dis < closest:
                    closest = dis
            dis = closest
#                 print(dis)
            bucket_id = math.ceil(dis/unit)
            covered.add(str(label)+'-'+str(i)+'-'+str(bucket_id))
...
#             print("y", Y[index])
            ...
            for i, path in enumerate(intra_paths):
                dises, dis = self.cal_path_similaity(path_x[index], path, simi_soft=simi_soft)
                if i == 0 or dis < closest:
                    closest = dis
                    closest_dises = dises
            for j, layer_dis in enumerate(dises):
                bucket_id = math.ceil(layer_dis / unit)
                covered.add(str(label)+'-'+str(i)+'-'+str(j)+'-'+str(bucket_id))
        return covered, total
